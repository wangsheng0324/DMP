...
object App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("APP")
      ...
      .set("es.index.auto.create", GlobalConfigUtils.autoCreateIndex) // 自动创建索引
      .set("es.nodes", GlobalConfigUtils.esNodes)
      .set("es.port", GlobalConfigUtils.esPort)
      .set("es.index.reads.missing.as.empty", GlobalConfigUtils.isMissing) // 索引不存在是否继续执行
      .set("es.nodes.discovery", GlobalConfigUtils.esNodesDiscovery) // 集群节点自动发现
      .set("es.nodes.wan.only", GlobalConfigUtils.wanOnly) // 是否只使用这个节点
